...
class data():
    ...
    def load(self):
        ...
        self.geneID = self.fe['proteins']['protein_name'][:]

    def getGeneIDs(self):
        """ get geneIDs """
        _chr = self.fe['proteins']['chrom'][:]
    ...
    def getGeneExpression(self,geneID,standardize=False):
        ...
        Y = self.fe['proteins']['proteins'][idx,:].T
        if standardize:
            Y-=Y.mean(0)
            Y/=Y.std(0)
        return Y
    ...
    def getGenePos(self,geneID):
        ...
        idx=self.geneID.index(geneID)
        gene_chrom = float(self.fe['proteins']['chrom'][idx])
        gene_start = float(self.fe['proteins']['gene_start_bp'][idx])
        gene_end = float(self.fe['proteins']['gene_stop_bp'][idx])
